...
public class BlockITBase<E extends Enum<E> & BlockITBase.IBlockEnum> extends Block implements IIEMetaBlock {
	...
	public BlockITBase(String name, Material material, PropertyEnum<E> mainProperty,
	...
	public IBlockState getInventoryState(int meta) {
		...
		// !this.additionalProperties[i].getAllowedValues().isEmpty())
		// state = state.withProperty(this.additionalProperties[i],
		// this.additionalProperties[i].getAllowedValues().toArray()[0]);
		return state;
	}
	...
	protected IBlockState getInitDefaultState() {
		...
		for (int i = 0; i < this.additionalProperties.length; i++)
			if (this.additionalProperties[i] != null && !this.additionalProperties[i].getAllowedValues().isEmpty())
				state = applyProperty(state, additionalProperties[i],
						additionalProperties[i].getAllowedValues().iterator().next());
		return state;
	}
	...
	public boolean canIEBlockBePlaced(World world, BlockPos pos, IBlockState newState, EnumFacing side, float hitX,
			float hitY, float hitZ, EntityPlayer player, ItemStack stack) {
		return true;
	}
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess world, BlockPos pos) {
		for (int i = 0; i < this.additionalProperties.length; i++)
			if (this.additionalProperties[i] != null && !this.additionalProperties[i].getAllowedValues().isEmpty())
				state = applyProperty(state, this.additionalProperties[i],
						this.additionalProperties[i].getAllowedValues().toArray()[0]);
		return state;
	}
	...
	void adjustSound() {
		...
		else if (this.material == Material.GRASS || this.material == Material.TNT
				|| this.material == Material.PLANTS || this.material == Material.VINE)
	...
	public abstract static class IPLadderBlock<E extends Enum<E> & IBlockEnum> extends BlockITBase<E> {
		...
		public void onEntityCollision(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
			...
			if (entityIn instanceof EntityLivingBase && !((EntityLivingBase) entityIn).isOnLadder()
					&& isLadder(state, worldIn, pos, (EntityLivingBase) entityIn)) {
				float f5 = 0.15F;
