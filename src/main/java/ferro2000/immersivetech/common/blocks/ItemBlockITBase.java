...
public class ItemBlockITBase extends ItemBlock {
	...
	public void addInformation(ItemStack stack, World worldIn, List<String> list, ITooltipFlag advInfo) {
		if (((BlockITBase) block).hasFlavour(stack)) {
			String subName = ((BlockITBase) this.block).getStateFromMeta(stack.getItemDamage())
					.getValue(((BlockITBase) this.block).property).toString().toLowerCase(Locale.US);
			String flavourKey = "desc." + ImmersiveTech.MODID + ".flavor." + ((BlockITBase) this.block).name + "."
					+ subName;
			list.add(TextFormatting.GRAY.toString() + I18n.format(flavourKey));
		}
		super.addInformation(stack, worldIn, list, advInfo);
		if (ItemNBTHelper.hasKey(stack, "energyStorage"))
			list.add(I18n.format("desc.immersiveengineering.info.energyStored",
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, BlockPos pos, EnumFacing side,
		...
		if (!((BlockITBase) this.block).canIEBlockBePlaced(world, pos, newState, side, hitX, hitY, hitZ, player, stack))
			return false;
		boolean ret = super.placeBlockAt(stack, player, world, pos, side, hitX, hitY, hitZ, newState);
		if (ret) {
			((BlockITBase) this.block).onIEBlockPlacedBy(world, pos, newState, side, hitX, hitY, hitZ, player, stack);
		}
		return ret;
	}
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing side,
		...
		if (!block.isReplaceable(world, pos))
			pos = pos.offset(side);
		if (stack.getCount() > 0 && player.canPlayerEdit(pos, side, stack)
				&& canBlockBePlaced(world, pos, side, stack)) {
	...
	private boolean canBlockBePlaced(World w, BlockPos pos, EnumFacing side, ItemStack stack) {
		...
		AxisAlignedBB axisalignedbb = blockIn.getCollisionBoundingBox(blockIn.getStateFromMeta(stack.getItemDamage()),
				w, pos);
